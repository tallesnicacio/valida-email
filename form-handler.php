...
if (!empty($_POST))
	{
		...
		$headers .= "De: \"".$from_name."\" <".$from_email.">\r\n";
		$headers .= "Responder para: \"".$from_name."\" <".$from_email.">\r\n";
		$message = $title."<br>";
		...
		if(!empty($_POST['firstname'])){
			$message .= "<b>Primeiro Nome:</b> ".$_POST['firstname'].'<br />';
			$MailChimpContact["FNAME"] = $_POST['firstname'];
		}					
		if(!empty($_POST['lastname'])){
			$message .= "<b>Sobrenome:</b> ".$_POST['lastname'].'<br />';
		...
		if(!empty($_POST['username'])){
			$message .= "<b>Usuário:</b> ".$_POST['username'].'<br />';
			$MailChimpContact["FNAME"] = (empty($MailChimpContact["FNAME"]))?$_POST['username']:NULL;
		}				
		if(!empty($_POST['username2'])){
			$message .= "<b>Usuário:</b> ".$_POST['username2'].'<br />';
			$MailChimpContact["LNAME"] = (empty($MailChimpContact["LNAME"]))?$_POST['username2']:NULL;
		}				
		if(!empty($_POST['password'])){ 	$message .= "<b>Senha:</b> ".$_POST['password'].'<br />';	}				
		if(!empty($_POST['password2'])){ 	$message .= "<b>Senha:</b> ".$_POST['password2'].'<br />';	}				
		if(!empty($_POST['budget'])){ 		$message .= "<b>Orçamento:</b> ".$_POST['budget'].'<br />';	}				
		if(!empty($_POST['company_size'])){ $message .= "<b>Tamanho da Empresa:</b> ".$_POST['company_size'].'<br />';	}				
		if(!empty($_POST['card'])){ 		$message .= "<b>Número do Cartão:</b> ".$_POST['card'].'<br />';	}				
		if(!empty($_POST['exp'])){ 			$message .= "<b>Data de expiração:</b> ".$_POST['exp'].'<br />';	}				
		if(!empty($_POST['cvv'])){ 			$message .= "<b>Código de Segurança:</b> ".$_POST['cvv'].'<br />';	}				
		if(!empty($_POST['zip'])){
			$message .= "<b>CEP:</b> ".$_POST['zip'].'<br />';
			$MailChimpContact["ADDRESS"]['zip'] = $_POST['zip'];
		}				
		if(!empty($_POST['country'])){
			$message .= "<b>País:</b> ".$_POST['country'].'<br />';
			$MailChimpContact["ADDRESS"]['country'] = $_POST['country'];
		}				
		if(!empty($_POST['city'])){
			$message .= "<b>Cidade:</b> ".$_POST['city'].'<br />';
			$MailChimpContact["ADDRESS"]['city'] = $_POST['city'];
		}				
		if(!empty($_POST['address'])){
			$message .= "<b>Endereço:</b> ".$_POST['address'].'<br />';
			$MailChimpContact["ADDRESS"]['addr1'] = $_POST['address'];
		}				
		if(!empty($_POST['message'])){ 		$message .= "<b>Mensagem:</b> ".str_replace("\n", "<br />", $_POST['message']).'<br />'; }
		if(!empty($_POST['send_copy'])){ 	$message .= "<b>Usuário marcou o campo:</b> Me envie uma cópia<br />";	}				
		if(!empty($_POST['remember'])){ 	$message .= "<b>Usuário marcou o campo:</b> Lembrar-me<br />";	}				
		if(!empty($_POST['rules'])){ 		$message .= "<b>Usuário marcou o campo:</b> Concordo com os Termos de Serviço<br />";	}				
		if(!empty($_POST['method'])){ 		$message .= "<b>Forma de Pagamento:</b> ".$_POST['method'].'<br />';	}				
		if(!empty($_POST['coupon'])){ 		$message .= "<b>Código do Cupom:</b> ".$_POST['coupon'].'<br />';	}				
